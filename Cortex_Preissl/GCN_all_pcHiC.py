...
if __name__ == '__main__':
    time_start = time()
    path_data_root = '/root/scATAC/ATAC_data/Forebrain/human_brain_all_pcHiC'
    ...
    # PLAC
    path_hic = '/root/scATAC/pcHi-C/all_pcHiC/hg38'
    ...
    for data in test_loader:
        ...
        mask = ig.attribute(
            input_mask, target=pred, n_steps=50,
            additional_forward_args=(data, model),
    ...
    adata_edge = ad.AnnData(X=df_weight_0, obs=dataset_ATAC.adata.obs.loc[df_weight_0.index, :])
    ...
    sc.tl.rank_genes_groups(adata_edge, groupby='celltype', method='wilcoxon', use_raw=False,
    ...
    df_pts = adata_edge.uns['rank_genes_groups']['pts']
    df_pts_rest = adata_edge.uns['rank_genes_groups']['pts_rest']
    df_pts_diff = df_pts - df_pts_rest

    df_graph = dataset_ATAC.df_graph
    peaks = dataset_ATAC.array_peak
    celltypes = df_pts.columns
    ...
    for idx_cell in range(len(celltypes)):
        ...
        for idx_edge in range(array_names.shape[0]):
            ...
            list_cell_pts.append(df_pts.loc[[edge_name], celltypes[idx_cell]])
            list_rest_pts.append(df_pts_rest.loc[[edge_name], celltypes[idx_cell]])
            list_pts_diff.append(df_pts_diff.loc[[edge_name], celltypes[idx_cell]])
        dict_cell_scores[cell_name] = pd.Series(list_edge_scores, index=list_edge_names)
        dict_cell_pts[cell_name] = pd.Series(list_cell_pts, index=list_edge_names)
        dict_rest_pts[cell_name] = pd.Series(list_rest_pts, index=list_edge_names)
        dict_pts_diff[cell_name] = pd.Series(list_pts_diff, index=list_edge_names)
    ...
    if not os.path.exists(path_cell_interatome):
        os.mkdir(path_cell_interatome)
    ...
    df_cell_sub = df_cell.loc[np.sum(df_cell, axis=1) != 0, :]
    ...
    for cell in list_cell:
        ...
        for celltype in celltypes:
            all_score = dict_cell_scores[celltype].loc[df_cell_sub.index]
    ...
    dict_promoter = {}
    file_peak_promoter = \
        '/root/scATAC/ATAC_data/Forebrain/human_brain_all_pcHiC/processed_files/peaks_promoter.txt'
