    ...
    def generate_peaks_file(self):
        ...
        with open(file_peaks, 'w') as w_peak:
            for one_peak in self.adata.var.index:
                ...
                start = one_peak.strip().split('-')[1]
                end = one_peak.strip().split('-')[2]
                peak_id = one_peak
...
def merge_per_cluster(adata_Control_raw, cell_merge, cutoff_prop, one_cluster):
    ...
    if adata_cluster.obs.shape[0] <= 15:
        ...
        sc.pp.highly_variable_genes(adata_cluster, n_top_genes=8000, flavor='seurat')
...
if __name__ == '__main__':
    ...
    dataset_AD_Control = ATACDataset(data_root=path_AD_Control,
                                     raw_filename='AD_human_brain_ATAC.h5ad')
    ...
    for celltype in celltypes:
        ...
        df_pts = adata_edge.uns['rank_genes_groups']['pts'].loc[array_names, :]
        df_pts['diff'] = df_pts[f'AD_{celltype}'] - df_pts[f'Control_{celltype}']
        df_MG = pd.DataFrame(
            {"edge_name": [(peaks[edge_name[0]], peaks[edge_name[1]]) for edge_name in array_names],
             ...
             "AD": df_pts[f'AD_{celltype}'], "Control": df_pts[f'Control_{celltype}'],
             "diff": df_pts['diff']},
        )
    ...
    # dict promoter
    dict_promoter = {}
    dict_gene_peak = defaultdict(list)
    file_peak_promoter = \
        '/root/scATAC/ATAC_data/Alzheimer_Morabito/AD_Control/processed_files/peaks_promoter.txt'
    with open(file_peak_promoter, 'r') as w_pro:
        for line in w_pro:
            ...
            if gene != '.':
                ...
                dict_gene_peak[gene].append(peak)
    ...
    path_inter_plot = os.path.join(path_ATAC_AD, 'inter_plot')
    if not os.path.exists(path_inter_plot):
        os.mkdir(path_inter_plot)
    # MG
    df_MG = dict_diff['MG']
    # INPP5D
    df_INPP5D = df_MG.loc[df_MG['gene'] == 'INPP5D', :]
    df_INPP5D.to_csv(os.path.join(path_inter_plot, 'INPP5D.txt'), sep='\t', )
    # USP6NL
    df_USP6NL = df_MG.loc[df_MG['gene'] == 'USP6NL', :]
    cre_gene = [gene for gene in df_USP6NL['cRE'] if gene[:3] != 'chr']
    for gene in cre_gene:
        df_tmp = df_USP6NL.loc[df_USP6NL['gene'] == gene, :]
        peaks = dict_gene_peak[gene]
        for peak in peaks:
            df_USP6NL = pd.concat([df_USP6NL, df_tmp])
            df_USP6NL.iloc[-1, 5] = peak
        df_USP6NL = df_USP6NL.loc[df_USP6NL['cRE'] != gene, :]
    df_USP6NL.to_csv(os.path.join(path_inter_plot, 'USP6NL.txt'), sep='\t', )
