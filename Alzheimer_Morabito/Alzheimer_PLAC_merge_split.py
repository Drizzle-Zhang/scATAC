...
if __name__ == '__main__':
    ...
    dataset_AD.adata = dataset_AD.adata[cells_AD, :]
    dataset_AD.adata_merge = dataset_AD.adata_merge[cells_AD, :]
    dataset_AD.generate_data_list()
    list_graph_data_AD = dataset_AD.list_graph
    path_graph_input_AD = os.path.join(path_AD_Control, 'input_graph_AD')
    os.system(f"rm -rf {path_graph_input_AD}")
    os.mkdir(path_graph_input_AD)
    dataset_atac_graph_AD = ATACGraphDataset(path_graph_input_AD, list_graph_data_AD)

    # dataset_atac_graph_AD.find_neighbors()
    # dataset_atac_graph_AD.plot_umap()
    ...
    sc.pl.umap(adata_Control, color=['nb_features', 'celltype'])

    # rank edge
    sc.tl.rank_genes_groups(adata_edge_Control, groupby='celltype', method='wilcoxon', use_raw=True,
                            pts=True, tie_correct=True)
    array_names = adata_edge_Control.uns['rank_genes_groups']['names']
    array_scores = adata_edge_Control.uns['rank_genes_groups']['scores']
    array_pval = adata_edge_Control.uns['rank_genes_groups']['pvals_adj']
