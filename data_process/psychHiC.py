...
# PO
df_PP_intersect = pd.read_csv(file_PP_intersect, sep='\t', header=None)
df_PO_intersect = df_PP_intersect.loc[df_PP_intersect.iloc[:, 4] == '.', :]
df_PO_intersect.iloc[:, 4] = df_PO_intersect.iloc[:, 3]
df_PO_intersect.iloc[:, 3] = df_PO_intersect.apply(lambda x: f"{x[0]}:{x[1]}-{x[2]}", axis=1)
df_PO = df_PO_intersect.iloc[:, [0, 1, 2, 3, 4]]
file_PO = os.path.join(path_hg38, 'PO.txt')
df_PO.to_csv(file_PO, sep='\t', index=False, header=False)
